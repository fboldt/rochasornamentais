...
df_authors['lista_revistas'] = None
df_authors['pares_revistas'] = None
...
for i, row in df_authors.iterrows():
    ...
    if not row['references_articles'].empty:
        for j, linha in row['references_articles'].iterrows():
            if len(linha['revista']) >= 3:
                todas_revistas.append(linha['revista'])
        todas_revistas = sorted(list(dict.fromkeys(todas_revistas)))
        row['lista_revistas'] = todas_revistas

    if row['lista_revistas'] != None:
        row['pares_revistas'] = list(
            itertools.combinations(row['lista_revistas'], 2))

    if row['pares_revistas'] != None:
        vertices += row['pares_revistas']
...
plt.tight_layout()
plt.savefig("./plot/cocitacao_fonte_periodicos.png", format="PNG")
# plt.show()
...
plt.tight_layout()
plt.savefig("./plot/cocitacao_fonte_periodicos_comunity.png", format="PNG")
# plt.show()